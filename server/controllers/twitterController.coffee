...
class TwitterController
  ...
  get: (url, req, callback) ->
    callback 'no twitter session' unless req.body.twitter?
    @consumer.get url, req.body.twitter.accessToken, req.body.twitter.accessTokenSecret,
    (err, data, response) ->
      callback err, data, response

  getJSON: (apiPath, req, callback) ->
    callback 'no twitter session' if !req.body.twitter
    @consumer.get "http://api.twitter.com/1#{apiPath}", req.body.twitter.accessToken, req.body.twitter.accessTokenSecret,
    (err, data, response) ->
      console.log data
      console.log "dataend"
  ...
  postJSON: (apiPath, body, req, callback) ->
    ...
    @consumer.post url, req.body.twitter.accessToken, req.body.twitter.accessTokenSecret, body,
    (err, data, response) ->
      callback err, JSON.parse(data), response
